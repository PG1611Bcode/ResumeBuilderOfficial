...
router.post('/analyze', auth, upload.single('cv'), async (req, res) => { 
  try {
    ...
    if (matchingSkills.length > 0) {
      strengths.push({ point: 'Good Skill Match', detail: `✅ Possesses ${matchingSkills.length} out of ${matchingSkills.length + missingSkills.length} required skills` });
    }
    if (parsedCV.experience && parsedCV.experience.length > 0) {
      strengths.push({ point: 'Relevant Experience', detail: `✅ Has ${parsedCV.experience.length} relevant work experience entries` });
    }
    if (parsedCV.education && parsedCV.education.length > 0) {
      strengths.push({ point: 'Solid Education', detail: `✅ Educational background in relevant field` });
    }
    if (parsedCV.skills && parsedCV.skills.length > 5) {
      strengths.push({ point: 'Broad Skill Set', detail: `✅ Strong technical skill set with ${parsedCV.skills.length} skills listed` });
    }
    if (score >= 60) {
      strengths.push({ point: 'Good Alignment', detail: `✅ Good alignment with ${role} role requirements` });
    }

    const weaknesses = [];
    if (missingSkills.length > 0) {
      weaknesses.push({ point: 'Missing Required Skills', detail: `⚠️ Missing ${missingSkills.length} key skills: ${missingSkills.slice(0, 3).join(', ')}${missingSkills.length > 3 ? '...' : ''}`, actionableStep: `Learn and add: ${missingSkills.slice(0, 3).join(', ')}` });
    }
    if (score < 50) {
      weaknesses.push({ point: 'Low Match Score', detail: `⚠️ Skills match score is below 50% - significant upskilling needed`, actionableStep: 'Focus on acquiring the core skills listed in the job description.' });
    }
    if (!parsedCV.certifications || parsedCV.certifications.length === 0) {
      weaknesses.push({ point: 'No Certifications', detail: '⚠️ No professional certifications mentioned', actionableStep: 'Consider getting a relevant certification.' });
    }
    if (parsedCV.experience && parsedCV.experience.length < 2) {
      weaknesses.push({ point: 'Limited Experience', detail: '⚠️ Limited work experience demonstrated', actionableStep: 'Add personal projects or internships to demonstrate your skills.' });
    }
